...
async function loadCurrentUser() {
    try {
        ...
        if (user && user.branchName) {
            ...
            document.getElementById('page-title').textContent = `Danh mục phòng của chi nhánh ${user.branchName}`;
        }
    } catch (error) {
...
async function handleCreateRoom(event) {
    ...
    try {
        const response = await callAPI('/rooms', 'POST', roomData);

        showSuccess(response.message || 'Tạo phòng mới thành công!');

        // Close modal and reload rooms
        ModalManager.close('create-room-modal');
        event.target.reset();

        // Reload with current filters if active
        if (hasActiveFilters()) {
            applyFilters();
        } else {
            loadRooms();
        ...
        showError(error.message || 'Không thể tạo phòng!');
    }
}
